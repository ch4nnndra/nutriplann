  ...
  <div class="flex min-h-screen">
    ...
    <div class="flex-1 p-6 px-7 bg-bg2">
      <div class="mb-5">
        ...
        <div class="text-[12px] text-tx2 mt-0.5">Selamat datang, {{ store.expertData?.name }}</div>
      </div>
      ...
      <div class="grid grid-cols-4 gap-3 mb-5">
        ...
        <div class="card">
          ...
          <div class="text-[28px] font-semibold leading-none mb-1 text-p">{{ store.expertStatistics.count }}</div>
          <div class="text-[11px] text-tx2 font-medium uppercase tracking-wider">Data referensi</div>
        </div>
        <div class="card">
          <div
            class="w-9 h-9 rounded-r8 flex items-center justify-center text-[18px] mb-2.5 bg-[#E6F1FB] text-[#185FA5]">
      ...
      <div class="grid grid-cols-2 gap-4" v-if="store.expertStatistics">
        <div class="card">
          <div class="card-t">
            <IconChartBar size="16" /> Distribusi program diet
          </div>
          <div class="mb-2.5" v-for="(distribution, i) in store.expertStatistics.distribution" :key="i">
            <div class="text-[12px] font-medium text-tx2 mb-1 flex justify-between">
              <span>{{ humanizeLabel(distribution.label) }} Diet</span>
              <span>{{ distribution.percentage }}%</span>
            </div>
            <div class="h-[7px] bg-bg3 rounded-full overflow-hidden">
              <div class="h-full bg-p400" :style="{ width: distribution.percentage + '%' }"></div>
            </div>
          </div>
          <!-- <div class="mb-2.5">
            <div class="text-[12px] font-medium text-tx2 mb-1 flex justify-between"><span>Diet Rendah
                Sodium</span><span>32%</span></div>
            <div class="h-[7px] bg-bg3 rounded-full overflow-hidden">
              <div class="h-full bg-p400" style="width: 32%"></div>
            </div>
          </div>
          <div class="mb-2.5">
            <div class="text-[12px] font-medium text-tx2 mb-1 flex justify-between"><span>Diet Rendah
                Karbohidrat</span><span>26%</span></div>
            <div class="h-[7px] bg-bg3 rounded-full overflow-hidden">
              <div class="h-full bg-p400" style="width: 26%"></div>
            </div>
          </div> -->
        </div>
        ...
        <div class="card">
          <div class="card-t justify-between">
            <span>
              <IconClock size="16" class="inline" /> Kasus terbaru menunggu tinjauan
            </span>
          ...
          <table v-else class="w-full text-[13px] border-collapse">
            <thead>
              <tr>
                <th
                  class="px-2 py-2 text-left text-[11px] font-medium text-tx2 bg-bg2 border-b border-bd uppercase tracking-wider">
                  ID</th>
                <th
                  class="px-2 py-2 text-left text-[11px] font-medium text-tx2 bg-bg2 border-b border-bd uppercase tracking-wider">
                  Kondisi</th>
            ...
            <tbody>
              <tr v-for="c in store.pendingCases.slice(0, 5)" :key="c.patient_id" class="hover:bg-bg2">
                <td class="px-2 py-2 border-b border-bd font-mono text-[11px]">{{ c.patient_id }}</td>
                <td class="px-2 py-2 border-b border-bd capitalize"><span class="badge b-red">{{
                  c.disease_type }}</span></td>
                <td class="px-2 py-2 border-b border-bd"><button class="btn btn-sm btn-p"
                    @click="router.push(`/expert/detail/${c.patient_id}`)">Tinjau</button></td>
              </tr>
            </tbody>
          </table>
        </div>
...
onMounted(async () => {
  ...
  console.log(store.expertStatistics);
})
</script>
